...
export default function TimeshareDetailPage() {
  ...
  if (!item) return <p className="text-gray-600">Listing not found.</p>;
  ...
  return (
    <div className="max-w-4xl mx-auto p-6 space-y-8">
        ...
        {storefront && (
          <div className="mt-4 space-y-3">
            ...
            <div className="flex items-center gap-4">
              <button
                onClick={() => router.push(`/storefront/${item.storeId}`)}
              ...
              <button
                onClick={() =>
                  router.push(`/messages/new?storeId=${item.storeId}`)
                }
      ...
      <div className="grid grid-cols-2 md:grid-cols-3 gap-4 text-gray-800">
        <div>
          <p className="font-semibold">Interval Type</p>
          <p className="capitalize">{item.intervalType}</p>
        </div>
        ...
        <div>
          ...
          <p className="capitalize">{item.season}</p>
        </div>

        <div>
          <p className="font-semibold">Maintenance Fees</p>
          <p>${item.maintenanceFees?.toLocaleString()}</p>
        </div>

        <div>
          <p className="font-semibold">Resort Rating</p>
          <p>{item.resortRating}</p>
        </div>
